...
- (void)awakeFromNib
{
    ...
    self.cellTopLabel.font = [UIFont boldSystemFontOfSize:14.0f];
    self.cellTopLabel.textColor = [UIColor lightGrayColor];

    self.messageBubbleTopLabel.font = [UIFont systemFontOfSize:14.0f];
...
//    NSLog(@"textView.font：%@，customAttributes.messageBubbleFont：%@",self.textView.font,customAttributes.messageBubbleFont);
...
- (void)setMediaView:(UIView *)mediaView
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        for (NSUInteger i = 0; i < self.messageBubbleContainerView.subviews.count; i++) {
            if (self.messageBubbleContainerView.subviews[i] != self->_mediaView) {
